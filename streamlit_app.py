...
def augment_image_with_bboxes(image, bboxes):
    ...
    augmented = augmentations(image=image, bboxes=bboxes, category=["face", "hand", "hand2"])
    return augmented['image'], augmented['bboxes']

# Draw bounding boxes on image
def draw_bounding_boxes(image, bboxes):
    overlay = image.copy()
    labels = ["Face", "Hand", "Hand2"][:len(bboxes)]
...
if uploaded_file:
    ...
    if st.button("Generate Synthetic Images"):
        ...
        for i, (img, bboxes) in enumerate(zip(synthetic_images, updated_bboxes)):
            ...
            metadata["images"].append({"file": img_path, "face_bbox": bboxes[0], "hand_bbox": bboxes[1], "hand2_bbox": bboxes[2] if len(bboxes) > 2 else None})
